import { Result, ok, err, fromPromise, fromThrowable } from 'neverthrow'
...
const getUser = async (userId: number): Promise<Result<User, UserNotFoundError>> => {
...
const safeFetch = (url: string) => fromPromise(
  fetchData(url),
...
const processUserData = async () => {
  ...
  updateResult.match(
    user => console.log(`Updated user: ${user.name} with email: ${user.email}`),
