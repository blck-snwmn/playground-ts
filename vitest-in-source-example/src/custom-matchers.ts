...
export function validateEmail(email: string): void {
  ...
  if (!email.includes("@")) {
    throw new ValidationError("Email must contain @ symbol", "email", "INVALID_FORMAT");
  }
...
export function fetchData(url: string): Promise<{ url: string; data: string; timestamp: number }> {
...
export function createUser(data: Partial<{ name: string; age: number; email: string }>) {
  return {
  ...
  describe("Advanced Error Handling and Assertions", () => {
    ...
    describe("Error handling patterns", () => {
      ...
      describe("Number parsing errors", () => {
        it("should handle parsing errors", () => {
          ...
          expect(() => parseNumber("abc")).toThrow('Cannot parse "abc" as number');
          expect(() => parseNumber("not-a-number")).toThrow(/Cannot parse .* as number/);
    ...
    describe("Advanced object matching", () => {
      ...
      it("should check array contents", () => {
        ...
        expect(data).toEqual(expect.arrayContaining([expect.objectContaining({ name: "Item 1" })]));
