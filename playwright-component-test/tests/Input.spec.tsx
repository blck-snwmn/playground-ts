...
test.describe("Input Component", () => {
  ...
  test("should render with label", async ({ mount }) => {
    const component = await mount(<Input label="Username" name="username" />);
  ...
  test("should show required indicator", async ({ mount }) => {
    const component = await mount(<Input label="Email" required />);
  ...
  test("should handle controlled value", async ({ mount }) => {
    ...
    const component = await mount(<Input value={currentValue} onChange={handleChange} />);
  ...
  test("should handle uncontrolled value with defaultValue", async ({ mount }) => {
    const component = await mount(<Input defaultValue="default text" />);
  ...
  test("should be readonly when readOnly prop is true", async ({ mount }) => {
    const component = await mount(<Input readOnly defaultValue="readonly text" />);
  ...
  test("should show error message", async ({ mount }) => {
    const component = await mount(<Input name="email" error="Invalid email address" />);
  ...
  test("should show helper text when no error", async ({ mount }) => {
    const component = await mount(<Input name="username" helperText="Enter your username" />);
  ...
  test("should not show helper text when error exists", async ({ mount }) => {
    const component = await mount(
      <Input name="field" error="Error message" helperText="Helper text" />,
    );
  ...
  test("should handle placeholder", async ({ mount }) => {
    const component = await mount(<Input placeholder="Enter text here" />);
  ...
  test("should handle maxLength attribute", async ({ mount }) => {
    const component = await mount(<Input maxLength={10} />);
  ...
  test("should handle focus and blur events", async ({ mount }) => {
    ...
    const component = await mount(
      <Input onFocus={() => events.push("focus")} onBlur={() => events.push("blur")} />,
    );
  ...
  test("should handle pattern attribute", async ({ mount }) => {
    const component = await mount(<Input pattern="[0-9]*" />);
  ...
  test("should clear input value", async ({ mount }) => {
    const component = await mount(<Input defaultValue="initial text" />);
