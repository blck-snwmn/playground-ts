...
test.describe("Counter Component", () => {
  ...
  test("should reset to initial count when Reset button is clicked", async ({ mount }) => {
    const component = await mount(<Counter initialCount={5} />);
  ...
  test("should call onCountChange callback when count changes", async ({ mount }) => {
    let capturedCount: number | undefined;
    const handleCountChange = (count: number) => {
      capturedCount = count;
    };

    const component = await mount(<Counter initialCount={0} onCountChange={handleCountChange} />);
