...
test.describe('TodoList Component', () => {
  ...
  test('should toggle todo completion', async ({ mount }) => {
    const component = await mount(<TodoList initialTodos={[
  ...
  test('should delete todo', async ({ mount }) => {
    const component = await mount(<TodoList initialTodos={[
      { id: '1', text: 'Todo to delete', completed: false, createdAt: new Date() }
    ]} />);
  ...
  test('should clear completed todos', async ({ mount }) => {
    const component = await mount(<TodoList initialTodos={[
      ...
      { id: '3', text: 'Completed 2', completed: true, createdAt: new Date() }
    ]} />);
  ...
  test('should load todos from localStorage', async ({ mount, page }) => {
    ...
    const testTodos = [
      { id: '1', text: 'Stored todo', completed: false, createdAt: new Date().toISOString() }
    ];
    
    // Set localStorage
    await page.evaluate(({ key, todos }) => {
