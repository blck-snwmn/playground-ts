...
test.describe("TodoList Component", () => {
	test("should render empty todo list", async ({ mount }) => {
		...
		await expect(
			component.getByRole("heading", { name: "Todo List" }),
		).toBeVisible();
		await expect(component.getByTestId("stats-total")).toContainText(
			"Total: 0",
		);
		await expect(component.getByTestId("stats-active")).toContainText(
			"Active: 0",
		);
		await expect(component.getByTestId("stats-completed")).toContainText(
			"Completed: 0",
		);
	});
	...
	test("should edit todo on double click", async ({ mount }) => {
		...
		await expect(component.getByTestId("todo-text-1")).toContainText(
			"Updated text",
		);
	});
	...
	test("should persist todos to localStorage", async ({ mount, page }) => {
		...
		const component = await mount(
			<TodoList enableLocalStorage={true} storageKey={storageKey} />,
		);
	...
	test("should handle multiple todos efficiently", async ({ mount }) => {
		...
		await component.getByTestId("filter-active").click();
		await expect(component.getByTestId("todo-list").locator("li")).toHaveCount(
			5,
		);
