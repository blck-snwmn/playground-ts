...
export function TodoList({
  ...
  const saveEdit = () => {
    if (editingId && editingText.trim()) {
      setTodos(
        todos.map((todo) => (todo.id === editingId ? { ...todo, text: editingText.trim() } : todo)),
      );
    }
    setEditingId(null);
    setEditingText("");
    ...
    <div className="todo-list">
      <header className="todo-header">
        ...
        <div className="todo-stats">
          ...
          <span data-testid="stats-completed">Completed: {stats.completed}</span>
        </div>
      </header>
          ...
          <li key={todo.id} className={`todo-item ${todo.completed ? "completed" : ""}`}>
            {editingId === todo.id ? (
              <div className="todo-edit">
                ...
                />
                <button type="button" onClick={saveEdit} data-testid={`save-btn-${todo.id}`}>
                  Save
                </button>
                <button type="button" onClick={cancelEdit} data-testid={`cancel-btn-${todo.id}`}>
                  Cancel
                </button>
              </div>
            ) : (
